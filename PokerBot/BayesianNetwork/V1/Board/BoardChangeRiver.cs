...
{
    public class BoardChangeRiver : BaseBoard
    {
        public BoardChangeRiver(Entity.Table.Board board) : base(board.getBoardFromRiver())
        {
            IEnumerable<PlayingCard> flopBoard = this._board.getBoardFromTurn().getEvaluator(); ;
            IEnumerable<PlayingCard> turnBoard = this._board.getBoardFromRiver().getEvaluator();
            ...
            {
                this.BoardEnumType = V1.Board.BoardEnumType.FullHouse;
            }
            else
            {
